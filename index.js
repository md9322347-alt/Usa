...
const TOKEN = '8024603369:AAHjOPI85yocArwEUySkd_jnxjZ1Pgb4qNc';
...
bot.on('photo', async (ctx) => {
  try {
    ...
    if (phones.length > 0) {
      // নাম্বারগুলো লাইন বাই লাইন
      const phoneListText = phones.join('\n');
      
      // ইনলাইন কীবোর্ড - শুধু একটি "কপি" বাটন
      const keyboard = {
        inline_keyboard: [[
          {
            text: "📋 কপি",
            callback_data: `copy_numbers_${Buffer.from(phoneListText).toString('base64')}`
          }
        ]]
      };
      
      const sentMsg = await ctx.reply(
        `📞 ${phones.length}টি নাম্বার:\n\`\`\`\n${phoneListText}\n\`\`\``,
        {
          parse_mode: 'Markdown',
...
// কলব্যাক কুয়েরি হ্যান্ডেল (কপি বাটনে ক্লিক করলে)
bot.on('callback_query', async (ctx) => {
  try {
    const callbackData = ctx.callbackQuery.data;
    
    if (callbackData.startsWith('copy_numbers_')) {
      const encodedText = callbackData.replace('copy_numbers_', '');
      const phoneListText = Buffer.from(encodedText, 'base64').toString('utf-8');
      
      // ইউজারকে জানাই যে কপি হয়েছে
      await ctx.answerCbQuery('✅ সব নাম্বার কপি হয়েছে!');
    }
  } catch (err) {
    console.error('Callback error:', err);
    ctx.answerCbQuery('❌ কপি করতে সমস্যা হয়েছে!').catch(() => {});
  }
});
